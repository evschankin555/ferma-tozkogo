...
app.controller('homeController', function( $scope, $rootScope, $http, $templateCache, $modal, $filter, commonService, ngTableParams, usSpinnerService ) {
    ...
    $rootScope.viewOrder = function( order_id, action ){
		...
		if(action == 'export')
		{
			var url=MODx.config.site_url+"exportorder.html";
									...
									$.ajax({
											...
											success: function(result){ if(result.error){alert(result.error);}
																	   if(result.success){window.open(result.success,"_self");}
																	   //alert(result);
																	 },
											error: function(x,err){alert(err);},
											dataType: 'json'          
										});
		}
